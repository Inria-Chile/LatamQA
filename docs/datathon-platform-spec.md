...
**Status:** Draft v0.4 — simplified scope. Changes from v0.3: (1) this document now covers
the **software only** — organizational, legal, staffing, and communication workstreams are
tracked by the organizers separately; (2) the **in-country reviewer** machinery is removed
(the validity checklist survives as a committee-wide check, §6.3); (3) the platform is
hosted as a **Hugging Face Space**, which replaces the single-VM Postgres/Argilla
architecture (§10) — Argilla is dropped and the built-in review UI becomes primary.
Decision history is retained in §11 with superseded items marked.
**Date:** 2026-07-04
...
- Automatic validation (schema, Wikidata links, language checks)
...
| `wikidata_qids` | One or more Wikidata entity IDs (`Q…`) — format checked at submit, existence verified asynchronously (§4) |
...
```text
draft ──submit──▶ submitted ──panel evaluation──▶ evaluated ──top-30 check──▶ accepted
                      │                               │
...
- **evaluated / accepted** — has a panel difficulty score; eligible for the top-30.
...
- Only the **top 30 by individual question score** count toward the team's final score.
...
2. Wikidata consistency, as a conjunction of conditionals: (`wikidata_qids` non-empty →
   every entry matches `Q\d+` format) **and** (`wikidata_qids` empty →
   `cultural_provenance` non-empty). QID **existence** is verified asynchronously
   (seconds later, like duplicate detection) via server-side batched `wbgetentities`
   calls against `www.wikidata.org` with a descriptive User-Agent; a nonexistent QID
   produces a soft flag, and **QID-only in-place edits on submitted questions are
   permitted** (audited; safe because QIDs are neither embedded for duplicate
   detection nor sent to the panel) — this soft flag is *not* the §3.1 `flagged` state
   and never affects quota or the refund window. A lookup timeout (~1 s) fails open
   with deferred re-verification. Rationale: Wikidata is a no-SLA third-party service;
   a maxlag episode must not block submissions in six countries at once (§9 degraded
   modes).
...
- Panel = **4 small-to-medium open-weight models** (O2/D9), configured with the
  existing `latamqa/models/*.yaml` format; served as **Hugging Face Inference
  Endpoints** (OpenAI-compatible API) called through LiteLLM — the YAML `LLM URI`
  field carries each endpoint URL. Rationale for small/medium: they are *more foolable*
  by cultural questions than 70B+ models (which often just know the answer), giving a
  wider score distribution that actually separates teams — an all-large panel would
  cluster scores near zero and discriminate poorly — and they are cheap and simple to
  serve.
...
- **Panel frozen at event open.** The composition is recorded in the event config and
  cannot be edited during the event. A failing endpoint is handled by deferral (§6.2 —
  errored cells re-queue), never by panel edits. **With only 4 models, one lost
  endpoint is 25 % of the signal**, so provision endpoint redundancy (Inference
  Endpoints support replica autoscaling) and treat a permanent model loss as
  significant: the final authoritative run then drops that model from **all** questions
  and recomputes every score from archived runs so every question shares one
  denominator (cheap by construction — recomputation from archived rows is arithmetic,
  no inference; the ~36 min/model figure of §6.2 applies only when a model's cells must
  actually be re-executed, e.g., endpoint-restore catch-up).
...
- **Errored runs are never scored** (divergence from a naive fixed denominator — and a
  ...
  ever errored. Otherwise an endpoint outage would count as "the model was fooled" and
  inflate scores by up to ~3 team points, unequally by submission time.
...
**~36 min/model** — with 4 per-model endpoints a full run is ~36 min wall-clock, so
rolling evaluation (§6.1) keeps up comfortably (each 15-minute cycle covers only
questions submitted since the last cycle), and the closing run covers the final cycle
plus errored-cell re-runs (budget ≈ 30–45 min between close and the country
announcements). Provision endpoint redundancy given each model is 25 % of the signal;
re-anchor the ~1 s constant on the chosen Inference Endpoint hardware once the 4
models are picked (O2).
...
(e.g., 10 % of all submissions), and post-hoc checks of every question that lands in a
team's top 30. During the event, the queue is **priority-ordered by the provisional
leaderboard**: flags on leading teams' questions first, then those teams' full top-30
sets — so whatever lands in a contender's top-30 is already validated by close, and the
same-day country announcement rests on a finished review rather than a close-time
scramble. The platform provides the ordering, per-question checklists, and review
progress views; how the committee is staffed is out of scope. The full top-30 audit
across all 72 teams (~2,160 questions) continues through Oct 5–20 and protects the
regional prize and the dataset. At the country freeze, a still-unadjudicated flag does
**not** suppress the question (presumption of validity); post-freeze exclusions affect
only the regional ranking and the export (decision D7, §7).
...
3. **Fails the validity checklist** (D17, revised — applied by any committee reviewer;
   ...
   obscure trivia, (d) sourced (Wikidata QID or accepted `cultural_provenance`). A
   question failing any item is excluded under this criterion. This check is mandatory
   for every top-30 question and subsumes reasons 2 and 9.
...
9. **Answer not uniquely correct** — a distractor is also true, or several options are
   ...
   **distractor-falsity verification** (each distractor affirmatively false, sourced
   via `wikidata_qids`/`context_note`) is a mandatory checklist item for every top-30
   check and for the same-day contender review.
...
participant rules (organizer task). The top-30 checklist also verifies the
...
- **Team score** (decision D6) = sum of `score(q)` over the team's top-30 accepted
  questions. This operationalizes the note's "nombre de questions qui parviennent à
  tromper le panel" as a normalized sum of fooling events (the note's Annexe grades
  questions by "score individuel"). D6, the D7 finality rule (including the presumption
  of validity for flags unadjudicated at the country freeze), and the full D10
  tie-break chain must all be mirrored verbatim in the published participant rules
...
- **Tie-breaking** (decision D10): pad every team's per-question score list with zeros
  ...
  submission-sequence sum over the counted top-30 set (zero-score counted questions
  included), computed at the freeze. Final backstop, unique by construction: the lowest
  single sequence number among counted questions — each sequence number belongs to
  exactly one team, so this cannot tie (cumulative sums *can*: disjoint sets can share
  a sum).
...
- **Regional leaderboard**: the country winners ranked by team score; winner announced
  ...
  (mean gap +2.5 accuracy points, worth ~0.75–2 team-score points). The organizers
...
- **Visibility during the event** (decision D5): country and regional leaderboards are
  ...
  provisional scores (aggregate only, §6.1), supporting the "submit 60, keep the best
  30" exploration loop.
...
- One-click admin export of all **accepted** questions to a Hugging Face dataset
  ...
  `wikidata_qids`, `cultural_provenance`, `team_id`, `event_id`, `panel_score`,
  `no_answer_rate`. The Space pushes directly to the Hub with its service token.
...
- **Availability:** the event is one day and cannot be rescheduled. The Space runs on
  ...
  latest snapshot) is **rehearsed at M4**. Degraded modes for four failure classes:
  (a) inference-endpoint loss — submissions accepted, evaluation deferred (§6.1);
  (b) Wikidata unavailability — QID existence checks defer, submissions flow (§4);
  (c) Space restart — persistent storage survives, background workers resume from the
  SQLite job queue (all jobs idempotent); (d) Space or storage loss — redeploy and
  restore the latest Hub snapshot per runbook.
...
- **Monitoring & alerting:** operators are alerted during the event when a team's
  ...
  external dependency (Wikidata, a panel endpoint) degrades.
...
Argilla is removed entirely — v0.3 had already demoted it to committee-review-only
...
└──────────────┘    │    cadence, backup sync                     │
 ...
 (all roles)                ▼                    ▼
              ┌──────────────────────┐  ┌──────────────────────────┐
              │ HF Inference          │  │ HF Hub                    │
              │ Endpoints:            │  │ · private repo: DB        │
              │ · 4 panel models      │  │   snapshots + eval        │
              │   (OpenAI-compatible, │  │   archives (5-min sync)   │
              │   via LiteLLM)        │  │ · public repo: LatamQA v2 │
              │ · e5 embeddings       │  │   dataset export          │
              │   (optional offload)  │  └──────────────────────────┘
              └──────────────────────┘
```
...
- **Auth:** HF OAuth for all roles (§2); Space secrets hold the service's Hub token and
  inference API keys.
- **Panel serving:** 4 HF Inference Endpoints (one per model, with replica redundancy),
  reached through LiteLLM as OpenAI-compatible endpoints — the existing YAML config's
  `LLM URI` field carries the endpoint URL, so the `latamqa` config format is reused
  unchanged. Embedding inference runs in-process on the Space CPU by default (the write
  load is tiny), with optional offload to a small Inference Endpoint.
...
(team, all §3 fields ×2 languages, status, **sequence_number**, uuid) · `embedding`
...
(reviewer, action, reason, note, at) · `score_snapshot` (team, run, top30_score,
...
- **D21 — Hosting (new):** one HF Docker Space, always-on hardware, persistent storage,
  ...
  panel models on HF Inference Endpoints via LiteLLM (§9, §10).
...
- **D6 — Score aggregation:** sum of continuous `score(q)` over the top-30 (§7).
...
- **D9 — Panel composition & disclosure (O2):** 4 small-to-medium open-weight models,
  ...
  endpoint sizing; re-anchor the §6.2 timing constant on the chosen hardware.
...
| --- | --- | --- |
| M0 — Space bootstrap | early Aug 2026 | Docker Space with HF OAuth, persistent storage, SQLite; round-trip a question end-to-end: form → validation → one Inference Endpoint evaluation via the `latamqa` programmatic API → score displayed. Prove the backup/restore cycle (snapshot to private Hub repo, restore to a fresh Space) |
...
| M2 — Evaluation | early Sep 2026 | Duplicate detection + orphan rule, panel evaluator (balanced permutations, errored-cell re-queue, archived seeds), rolling evaluation + provisional scores; **first load test** at canonical scale |
...
| M4 — Full rehearsal | ~Sep 18 2026 | Full-day rehearsal with synthetic teams incl. same-day contender review drill and Space restore drill; load test at 2× canonical scale; axe scan + assistive-tech walkthrough; ops runbook finalized |
